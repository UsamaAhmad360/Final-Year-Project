...
void checkWaterLevelAndPumpStatus() {
  ...
  if (automationEnabled) {
    if (waterLevelPercent <= 20 && !pumpRunning) {
    ...
    else if (waterLevelPercent < 80 && pumpRunning && flowRate == 0 && (currentMillis - pumpStartTime >= 5000)) {
...
//else if (pumpRunning && flowRate == 0 && (currentMillis - pumpStartTime >= 5000)) {
//      // If pump is running and no water flow detected within 5 seconds, turn off pump and wait for 1 minute
//      digitalWrite(relayPin, LOW); // Turn off pump
//      pumpRunning = false;
//      delay(60000); // Wait for 1 minute
//    }
