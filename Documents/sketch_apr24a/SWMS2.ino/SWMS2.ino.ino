...
void checkWaterLevelAndPumpStatus() {
  ...
  int waterLevel = map(smoothedDistance, maxWaterLevel, minWaterLevel, 100, 0);
  Blynk.virtualWrite(V2, waterLevel);

  if (automationEnabled && smoothedDistance < maxWaterLevel && !pumpRunning) {
    digitalWrite(relayPin, HIGH);
    pumpRunning = true;
    startTime = millis();
    Blynk.virtualWrite(V3, 1);
    Blynk.virtualWrite(V1, 1);
  } else if (automationEnabled && smoothedDistance >= maxWaterLevel && pumpRunning && (currentMillis - startTime >= 5000)) {
    digitalWrite(relayPin, LOW);
    pumpRunning = false;
    Blynk.virtualWrite(V3, 0);
    Blynk.virtualWrite(V1, 0);
