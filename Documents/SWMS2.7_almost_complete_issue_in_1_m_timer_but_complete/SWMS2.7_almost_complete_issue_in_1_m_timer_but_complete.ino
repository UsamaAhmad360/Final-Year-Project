...
unsigned long timerDuration = 60000; // 10 seconds timer duration

float smoothedDistance = 0.0;
float alpha = 0.1;

// Define thresholds and limits
const int minWaterLevel = 16;  // Minimum water level in cm
const int maxWaterLevel = 5; // Maximum water level in cm
...
void checkWaterLevelAndPumpStatus() {
  ...
  if (automationEnabled) {
    if (waterLevelPercent <= 20 && !pumpRunning && (currentMillis - previousTimerMillis >= timerDuration)) {
      // Start pump if water level is less than or equal to 20%, pump is not running, and timer duration has elapsed
      digitalWrite(relayPin, HIGH); // Turn on pump
      pumpRunning = true;
      startTime = millis();
      previousTimerMillis = currentMillis; // Reset timer
