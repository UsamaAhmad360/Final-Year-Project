...
void checkWaterLevelAndPumpStatus() {
  ...
  if (automationEnabled) {
 ...
 else if (waterLevelPercent < 100 && pumpRunning && flowRate>0) {
      // If pump is running and no water flow after 5 seconds, turn off pump
    ...
    else if (waterLevelPercent < 100 && pumpRunning && (currentMillis - startTime >= 5000)) {
      // If pump is running and no water flow after 5 seconds, turn off pump
      digitalWrite(relayPin, LOW); // Turn off pump
      pumpRunning = false;
      Blynk.virtualWrite(V3, 0); // Update relay state on Blynk
