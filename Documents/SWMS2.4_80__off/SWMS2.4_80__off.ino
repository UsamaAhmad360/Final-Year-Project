...
void checkWaterLevelAndPumpStatus() {
  ...
  if (automationEnabled) {
    if (waterLevelPercent == 100 && pumpRunning) {
      digitalWrite(relayPin, LOW); // Turn off pump
      pumpRunning = false;
      Blynk.virtualWrite(V3, 0); // Update relay state on Blynk
      Blynk.virtualWrite(V1, 0); // Update manual pump control button state to off
    } else if (waterLevelPercent < 80 && !pumpRunning) {
      // Start pump if water level is less than 80% and no water is being pumped
      digitalWrite(relayPin, HIGH); // Turn on pump
      pumpRunning = true;
      startTime = millis();
      Blynk.virtualWrite(V3, 1); // Update relay state on Blynk
      Blynk.virtualWrite(V1, 1); // Update manual pump control button state to on
    } else if (waterLevelPercent >= 80 && waterLevelPercent < 100 && pumpRunning && flowRate > 0 && (currentMillis - startTime >= 5000)) {
      // If pump is running, no water flow after 5 seconds, and water level is between 80% and 100%, turn off pump
      digitalWrite(relayPin, LOW); // Turn off pump
      pumpRunning = false;
      Blynk.virtualWrite(V3, 0); // Update relay state on Blynk
      Blynk.virtualWrite(V1, 0); // Update manual pump control button state to off
